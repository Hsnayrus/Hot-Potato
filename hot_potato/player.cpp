...
int main(int argc, char **argv) {
    if (argc != 3) {
        std::cerr << "Wrong usage. Correct usage is ./player machineName "
                     "pprtNumber\n";
        return 0;
    }
  ...
  */
    ...
    if (syncInfo.startAccepting == 1) {
        rightPlayer.connectTo(rightPlayer.getSocket_FD());
        std::cout << "Connected as player " << myID << " out of "
                  << info.noPlayers << " total players\n";
...
//                     ? ((playerClient_fd > playerRingMaster_fd) ?
//                     playerClient_fd
//                                                                :
//                                                                playerRingMaster_fd)
//                     : ((playerServer_fd > playerRingMaster_fd) ?
//                     playerServer_fd
//                                                                :
//                                                                playerRingMaster_fd);
