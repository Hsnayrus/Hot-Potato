...
int main(int argc, char ** argv) {
    if (argc != 4) {
        std::cerr << "Wrong usage. Correct usage is: ./ringmaster portnumber "
                     "numPlayers numHops\n";
        return 0;
    }
    ...
    // Then i will just assign target addresses to players starting from
    // index 1. This is a cleaner approach
    ...
    // Loop to send client's their portNumber, their fd's and their neighbors ip
    // Addresses
    for (size_t i = 0; i < clientsInformation.size(); i++) {
        ...
        temp.fd = clientsInformation[i].fd;
        // std::cout << "%%%%%%%%%%%%%%%%\n";
        // std::cout << temp.fd << " " << temp.ipSize << " " << temp.portNum <<
        // std::endl; for (int i = 0; i < temp.ipSize; i++) {
    ...
    // std::cout << "Starting game with player " << playerToStartWith <<
    // std::endl;
    ...
    std::cout << "Ready to start the game, sending potato to player "
              << playerToStartWith << std::endl;
    if (noHops != 0) {
        int status = send(clientsInformation[playerToStartWith].fd, &potato,
                          sizeof(potato), 0);
        if (status == -1) {
            std::cerr << "Couldn't send potato to player: " << playerToStartWith
                      << std::endl;
        }
        std::vector<int> fds;
...
//     > >(); noPlayers = 0; makeServerSocket();
//   }

//   RingMaster(size_t playerNos) : Server(Socket("4445")) {
//     players = std::vector<std::pair<int, std::pair<std::string, std::string>
//     > >(); assert(playerNos > 0); noPlayers = playerNos;
//   }
...
//   }
//   //This functino waits for incoming connections and assigns players
//   automatically/
//   //std::vector<std::pair<int, std::pair<std::string, std::string> > >
//   playersis in the format:
...
//       if (messageFromClient.find("Ready for connection.") !=
//       std::string::npos) {
//         std::string portStr = num2Str(port);
//         players.push_back(
//             std::make_pair(temp.first, std::make_pair(temp.second,
//             portStr)));
//       }
//     }
//     return true;
...
//       message.clear();
//       message =
//           players[i + 1].second.first + "//////" + players[i +
//           1].second.second + "\n";
...
//     send(players[players.size() - 1].first, buffer.data(), message.size(),
//     0);
//   }
